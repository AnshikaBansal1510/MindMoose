...
const isToday = (someDate) => {
  // checks whether a given data represents today
...
const calculateStreak = (completionDates) => {
  ...
  // if user nevery completed anything
  if (!completionDates.length) return 0;

  // Converts all dates into a uniform string format
  // Example: "Mon Jan 05 2026"...renoves time part
...
export const toggleHabitCompletion = async (req, res) => {
  ...
  try {
    ...
    // checks a habit is completed only once per day
    // isToday removes time
    // .some() return true if today date is present
